...
export default function Dashboard() {
  ...
  const [analyticsMetrics, setAnalyticsMetrics] = useState(null);   // NEW: server-side KPIs
  const [branchPerformance, setBranchPerformance] = useState([]);   // NEW: per-branch stats
  const [bestSellers, setBestSellers] = useState([]);               // NEW: top products
  ...
  useEffect(() => {
    ...
    if (!userData) {
      ...
      if (parsedUser.role === 'employee' || parsedUser.role === 'staff') {
        window.location.href = "/sales"; // Redirect staff away from dashboard
        return;
      }
      setUser(parsedUser);
      fetchAllData();
  ...
  const fetchAllData = async (days = dateFilter) => {
    ...
    try {
      ...
      // Parallel fetch — new analytics endpoints + legacy inventory/stock endpoints
      const [
  ...
  const predictedRevenue =
    currentDay > 0
      ? (currentRevenue / currentDay) * daysInMonth
      : 0;

  const targetAchieved = Math.min((currentRevenue / TARGET_REVENUE) * 100, 100);
  const targetRemaining = Math.max(100 - targetAchieved, 0);

  // Gauge colour — 5-tier scale matching the progress reference table
  let gaugeColor = '#EF4444'; // 0–30 % → Critical (red)
  if (targetAchieved >= 100) gaugeColor = '#10B981';      // 100%+ → Target Achieved (emerald)
  else if (targetAchieved >= 81) gaugeColor = '#22C55E';  // 81–99% → On Track (green)
  else if (targetAchieved >= 61) gaugeColor = '#84CC16';  // 61–80% → Good Progress (lime)
  else if (targetAchieved >= 31) gaugeColor = '#F59E0B';  // 31–60% → Needs Attention (amber)
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 bg-brand-bgbase text-main">
          <div className="responsive-container">
            ...
            {/* Row 1: KPI Cards — prefer server-side analytics, fall back to client-computed */}
            <div className="responsive-grid mb-8">
              <StatCard
                title="Total Revenue"
                value={`₱${(analyticsMetrics?.totalRevenue ?? kpis.revenue).toLocaleString()}`}
                icon={PesoSign}
                trend={user?.role === 'super_admin' ? "Live Global Total" : "Branch Total"}
              />
              <StatCard
                title="Total Stock"
                value={analyticsMetrics?.totalStock ?? (Array.isArray(inventory) ? inventory.reduce((s, i) => s + i.quantity, 0) : 0)}
                icon={Box}
                trend="Units On-Hand"
              />
              <StatCard
                title="System Orders"
                value={analyticsMetrics?.totalOrders ?? kpis.orders}
                icon={Package}
                trend={user?.role === 'super_admin' ? "System Wide" : "Branch Orders"}
              />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8" ref={trendsRef}>
              ...
              <div className="glass-card p-6 flex flex-col justify-center">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h3 className="font-rajdhani font-black text-xl uppercase tracking-widest">
                    Executive Summary
                  </h3>
                </div>
              ...
              <div className="glass-card p-6 mb-8">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="px-3 py-1 rounded-full bg-green-500/10 text-green-500 text-[10px] uppercase font-black tracking-widest">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-brand-bgbase/30 rounded-xl p-4">
                    <p className="text-[10px] uppercase font-black text-muted mb-2">
                      Current Revenue
                    </p>
                  ...
                  <div className="bg-brand-bgbase/30 rounded-xl p-4 md:col-span-2">
                    ...
                    <p className="text-sm font-bold text-brand-neonblue">
                      Revenue trend indicates positive growth trajectory.
                    </p>
                  </div>
              ...
              <div className="glass-card flex flex-col h-[400px] md:h-[510px]">
                ...
                <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar">
                  {salesHistory.slice(0, 10).map((order, i) => {
                    // Handle both new Sale model fields and legacy Order fields
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-[9px] font-black text-muted uppercase tracking-widest truncate">
                              {new Date(order.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} • {order.Branch?.name ?? '—'} • {invoice}
                            </p>
                          </div>
                        </div>
