...
import { Package, UploadCloud, Save, Image as ImageIcon, ArrowLeft, Loader2, CheckCircle, Trash2, X } from "lucide-react";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error(errorData.error || errorData.message || "Failed to create product");
        setLoading(false);
      }
    } catch (err) {
              ...
              <div className="bg-brand-surface border border-border/50 rounded-2xl p-6 md:p-8 shadow-sm">
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-[10px] font-black text-muted uppercase tracking-[2px] mb-2">SKU / Barcode *</label>
                      <input 
                        type="text" 
                        name="sku"
                        value={formData.sku}
                        onChange={handleChange}
                        placeholder="e.g. SKU-4090-FE" 
                        className="w-full bg-brand-bgbase border border-border/50 rounded-xl px-4 py-3 text-sm text-main font-bold outline-none focus:border-brand-neonblue transition-colors"
                        required
                      />
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-brand-surface border border-border/50 rounded-2xl p-6 shadow-sm">
                ...
                <div 
                  ...
                  className={`w-full aspect-square bg-brand-bgbase border-2 border-dashed rounded-xl flex flex-col items-center justify-center cursor-pointer transition-all relative overflow-hidden group ${
                    isDragging 
                      ? 'border-brand-neonblue bg-brand-neonblue/10 scale-[1.02]' 
                      : 'border-border/50 hover:border-brand-neonblue hover:bg-brand-neonblue/5'
                  }`}
                >
                  {imagePreview ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity gap-3">
                        ...
                        <button 
                          ...
                          className="text-white text-xs font-bold uppercase tracking-widest flex items-center gap-2 bg-brand-crimson/80 hover:bg-brand-crimson px-4 py-2 rounded-full shadow-lg transition-colors border border-brand-crimson/20"
                        >
              ...
              <div className="bg-brand-surface border border-border/50 rounded-2xl p-6 shadow-sm">
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    <label className="block text-[10px] font-black text-muted uppercase tracking-[2px] mb-2">Supplier / Brand</label>
                    <select 
                      name="supplier_id"
