...
export default function ExpensesPage() {
  ...
  const handleAdd = (e) => {
    e.preventDefault();
    if (!form.description || !form.amount) return toast.error("Fill in all required fields");
    const newEntry = {
      id: Date.now(),
      ...form,
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 custom-scrollbar relative z-10 bg-brand-bgbase text-main">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-6">
            ...
            <button
              ...
              className="btn-premium flex items-center gap-2 px-6 py-2.5 rounded-full text-xs"
            >
              <PlusCircle size={16} /> Log New Expense
            </button>
          </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
              ...
              className="w-full max-w-md bg-brand-surface border border-border rounded-[32px] p-8 relative z-10 shadow-2xl overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-brand-crimson/10 blur-[80px] pointer-events-none" />
              <div className="mb-8">
                <p className="text-[10px] font-black uppercase tracking-[3px] text-brand-crimson mb-1">New Entry</p>
                <h3 className="text-xl font-rajdhani font-black uppercase tracking-widest">Log Expense</h3>
              </div>
              <form onSubmit={handleAdd} className="space-y-4">
                <div>
                  <label className="text-[9px] font-black uppercase tracking-[3px] text-muted ml-1">Description *</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[9px] font-black uppercase tracking-[3px] text-muted ml-1">Amount (₱) *</label>
                    <input type="number" required min="0" step="0.01" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })}
                      placeholder="0.00"
                      className="w-full mt-1.5 bg-brand-bgbase border border-border rounded-2xl py-3 px-4 text-sm text-main focus:outline-none focus:border-brand-crimson transition-all" />
                  </div>
                  <div>
                    <label className="text-[9px] font-black uppercase tracking-[3px] text-muted ml-1">Date *</label>
                    <input type="date" required value={form.date} onChange={e => setForm({ ...form, date: e.target.value })}
                      className="w-full mt-1.5 bg-brand-bgbase border border-border rounded-2xl py-3 px-4 text-sm text-main focus:outline-none focus:border-brand-neonblue transition-all" />
                  </div>
                </div>
                <div>
                  <label className="text-[9px] font-black uppercase tracking-[3px] text-muted ml-1">Category</label>
                ...
                <div className="flex gap-3 pt-4">
                  <button type="button" onClick={() => setIsModalOpen(false)}
                    className="flex-1 py-3.5 rounded-full border border-border text-[10px] font-black uppercase tracking-[3px] text-muted hover:text-main transition-all">
                    Cancel
                  </button>
                  <button type="submit"
