            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.button
                  ...
                  className="h-12 px-7 flex items-center gap-2 bg-brand-crimson hover:bg-red-700 text-white rounded-full text-[10px] font-black uppercase tracking-[2px] shadow-[0_8px_20px_rgba(215,38,56,0.2)] transition-all"
                >
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="glass-card w-full max-w-lg p-8 md:p-10 relative z-10 shadow-[0_20px_80px_rgba(0,0,0,0.2)]"
            >
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h2 className="text-xl font-rajdhani font-black uppercase tracking-tight">
                    Add <span className="text-brand-neonblue">New Customer</span>
                  </h2>
                  <p className="text-[10px] text-muted font-bold uppercase tracking-[2px] mt-1 opacity-40">
                    Customer record will be saved to database
                  </p>
                </div>
                <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-brand-surface rounded-xl text-muted hover:text-main transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <form onSubmit={handleAddCustomer} className="space-y-5">
                {[
                  ...
                  { label: "Email Address", key: "email", type: "email", placeholder: "customer@email.com" },
                  { label: "Phone Number", key: "phone", type: "text", placeholder: "+63 9XX XXX XXXX" },
                  { label: "Address", key: "address", type: "text", placeholder: "City, Province" }
                ].map(field => (
                  <div key={field.key} className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-[2px] text-muted">{field.label}</label>
                    <input
                      type={field.type}
                      value={formData[field.key]}
                      onChange={e => setFormData({ ...formData, [field.key]: e.target.value })}
                      placeholder={field.placeholder}
                      className="w-full bg-brand-surface border border-border rounded-xl py-3.5 px-5 text-sm text-main focus:outline-none focus:border-brand-neonblue/40 transition-all font-bold placeholder:opacity-20"
                    />
                  </div>
                ))}
                ...
                <div className="pt-3">
                  <motion.button
                    ...
                    type="submit"
                    disabled={submitting}
                    className="w-full h-13 py-4 bg-brand-crimson hover:bg-red-700 text-white rounded-full text-[11px] font-black uppercase tracking-[3px] flex items-center justify-center gap-2 transition-all shadow-[0_8px_20px_rgba(215,38,56,0.2)] disabled:opacity-50"
                  >
