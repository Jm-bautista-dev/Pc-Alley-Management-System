...
export default function PurchaseSalePage() {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      toast.error("Network Link Offline");
    } finally {
      setLoading(false);
    }
  };
                     ...
                     {sales.slice(0, 5).map((sale) => (
                       <div key={sale.id} className="flex justify-between items-center bg-brand-bgbase/50 p-4 rounded-xl border border-border/30">
                         <div>
                           ...
                           <p className="text-[10px] font-black uppercase tracking-widest text-muted">{new Date(sale.createdAt).toLocaleDateString()} • {sale.OrderItems?.length} items</p>
                         </div>
                         <p className="text-sm font-rajdhani font-black text-green-500">+₱{Number(sale.total_amount).toLocaleString()}</p>
                       </div>
                     ))}
